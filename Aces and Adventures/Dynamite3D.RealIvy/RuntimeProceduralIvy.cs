...
public class RuntimeProceduralIvy : RTIvy
{
	...
	protected override void Init(IvyContainer ivyContainer, IvyParameters ivyParameters)
	{
		...
		rtIvyGrowth.Init(rtIvyContainer, ivyParameters, base.gameObject, leavesMeshesByChosenLeaf, GetMaxNumPoints(), GetMaxNumLeaves(), GetMaxNumVerticesPerLeaf());
		for (int i = 0; i < 10; i++)
		{
			rtIvyGrowth.Step();
		}
	...
	{
		meshBuilder.InitializeMeshesDataProcedural(bakedMesh, numBranches, growthParameters.lifetime, growthParameters.growthSpeed);
	}
	...
	{
		base.growthParameters = growthParameters;
