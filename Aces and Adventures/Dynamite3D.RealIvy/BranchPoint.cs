...
public class BranchPoint
{
	...
	{
		SetValues(point, grabVector, Vector3.zero, branchContainer, index, blocked: false, newBranch, newBranchNumber, length);
	}

	public BranchPoint(Vector3 point, Vector3 grabVector, int index, float length, BranchContainer branchContainer)
	{
		SetValues(point, grabVector, Vector3.zero, branchContainer, index, blocked: false, newBranch: false, -1, length);
	}

	public BranchPoint(Vector3 point, int index, float length, BranchContainer branchContainer)
	{
		SetValues(point, Vector3.zero, Vector3.zero, branchContainer, index, blocked: false, newBranch: false, -1, length);
	}
