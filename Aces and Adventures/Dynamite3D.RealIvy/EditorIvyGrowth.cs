...
public class EditorIvyGrowth : GrowthBuilder
{
	...
	public void Initialize(Vector3 firstPoint, Vector3 firstGrabVector)
	{
		...
		infoPool.ivyContainer.branches[0].AddBranchPoint(firstPoint, firstGrabVector, isNewBranch: true, branchContainer.branchNumber);
	...
	private void CalculateNewHeight(BranchContainer branch)
	{
		...
		branch.newHeight += (Mathf.Sin(branch.heightParameter * infoPool.ivyParameters.DTSFrequency * branch.randomizeHeight) + 1f) / 2f * infoPool.ivyParameters.maxDistanceToSurface / 4f * infoPool.ivyParameters.DTSRandomness;
	...
	{
		BranchPoint branchPoint = new BranchPoint(branch.GetLastBranchPoint().point + branch.growDirection * infoPool.ivyParameters.stepSize + branch.GetLastBranchPoint().grabVector * branch.deltaHeight, branch.branchPoints.Count, 0f, branch);
		Vector3 direction = branchPoint.point - branch.GetLastBranchPoint().point;
		if (!Physics.Raycast(new Ray(branch.branchPoints[branch.branchPoints.Count - 1].point, direction), out var hitInfo, infoPool.ivyParameters.stepSize * 1.15f, infoPool.ivyParameters.layerMask.value))
		{
	...
	{
		if (Physics.Raycast(new Ray(potentialPoint.point, -oldSurfaceNormal), out var hitInfo, branch.currentHeight * 2f, infoPool.ivyParameters.layerMask.value))
		{
			AddPoint(branch, hitInfo.point, hitInfo.normal);
		...
		else if (Random.value < infoPool.ivyParameters.grabProvabilityOnFall)
		{
	...
	{
		if (Physics.Raycast(new Ray(potentialPoint.point + branch.branchPoints[branch.branchPoints.Count - 1].grabVector * 2f * branch.currentHeight, -branch.growDirection), out var hitInfo, infoPool.ivyParameters.stepSize * 1.15f, infoPool.ivyParameters.layerMask.value))
		{
	...
	{
		if (!Physics.Raycast(new Ray(branch.branchPoints[branch.branchPoints.Count - 1].point, branch.growDirection), out var hitInfo, infoPool.ivyParameters.stepSize * 1.15f, infoPool.ivyParameters.layerMask.value))
		{
	...
	public void AddPoint(BranchContainer branch, Vector3 point, Vector3 normal)
	{
		...
		if (growing && Random.value < infoPool.ivyParameters.branchProvability && infoPool.ivyContainer.branches.Count < infoPool.ivyParameters.maxBranchs)
		{
	...
	private void AddFallingPoint(BranchContainer branch)
	{
		...
		branch.totalLenght += infoPool.ivyParameters.stepSize;
		branch.AddBranchPoint(branch.branchPoints[branch.branchPoints.Count - 1].point + branch.growDirection * infoPool.ivyParameters.stepSize, grabVector);
	...
	{
		branch.growDirection = Vector3.Normalize(Vector3.ProjectOnPlane(Quaternion.AngleAxis(Mathf.Sin((float)branch.branchSense * branch.totalLenght * infoPool.ivyParameters.directionFrequency * (1f + Random.Range(0f - infoPool.ivyParameters.directionRandomness, infoPool.ivyParameters.directionRandomness))) * infoPool.ivyParameters.directionAmplitude * infoPool.ivyParameters.stepSize * 10f * Mathf.Max(infoPool.ivyParameters.directionRandomness, 1f), branch.GetLastBranchPoint().grabVector) * branch.growDirection, branch.GetLastBranchPoint().grabVector));
	}
	...
	{
		Vector3 vector = Vector3.Lerp(branch.growDirection, infoPool.ivyParameters.gravity, branch.fallIteration / 10f);
		vector = Quaternion.AngleAxis(Mathf.Sin((float)branch.branchSense * branch.totalLenght * infoPool.ivyParameters.directionFrequency * (1f + Random.Range((0f - infoPool.ivyParameters.directionRandomness) / 8f, infoPool.ivyParameters.directionRandomness / 8f))) * infoPool.ivyParameters.directionAmplitude * infoPool.ivyParameters.stepSize * 5f * Mathf.Max(infoPool.ivyParameters.directionRandomness / 8f, 1f), branch.GetLastBranchPoint().grabVector) * vector;
		vector = Quaternion.AngleAxis(Mathf.Sin((float)branch.branchSense * branch.totalLenght * infoPool.ivyParameters.directionFrequency / 2f * (1f + Random.Range((0f - infoPool.ivyParameters.directionRandomness) / 8f, infoPool.ivyParameters.directionRandomness / 8f))) * infoPool.ivyParameters.directionAmplitude * infoPool.ivyParameters.stepSize * 5f * Mathf.Max(infoPool.ivyParameters.directionRandomness / 8f, 1f), Vector3.Cross(branch.GetLastBranchPoint().grabVector, branch.growDirection)) * vector;
	...
	private void Refine(BranchContainer branch)
	{
		...
		{
			if (Vector3.Distance(branch.branchPoints[branch.branchPoints.Count - 2].point, branch.branchPoints[branch.branchPoints.Count - 3].point) < infoPool.ivyParameters.stepSize * 0.7f || Vector3.Distance(branch.branchPoints[branch.branchPoints.Count - 2].point, branch.branchPoints[branch.branchPoints.Count - 1].point) < infoPool.ivyParameters.stepSize * 0.7f)
			{
			...
			{
				Vector3 vector = branch.branchPoints[branch.branchPoints.Count - 1].point - branch.branchPoints[branch.branchPoints.Count - 2].point;
				Vector3 vector2 = branch.branchPoints[branch.branchPoints.Count - 3].point - branch.branchPoints[branch.branchPoints.Count - 2].point;
				branch.InsertBranchPoint(branch.branchPoints[branch.branchPoints.Count - 2].point + vector2 / 2f, branch.branchPoints[branch.branchPoints.Count - 2].grabVector, branch.branchPoints.Count - 2);
				branch.InsertBranchPoint(branch.branchPoints[branch.branchPoints.Count - 2].point + vector / 2f, branch.branchPoints[branch.branchPoints.Count - 2].grabVector, branch.branchPoints.Count - 1);
