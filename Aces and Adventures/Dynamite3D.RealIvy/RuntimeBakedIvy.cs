...
public class RuntimeBakedIvy : RTIvy
{
	public override bool IsGrowingFinished()
	{
		...
		else
		{
			...
			{
				flag = flag && rtBuildingIvyContainer.branches[i].branchPoints.Count >= activeBakedBranches[i].branchPoints.Count;
			}
		}
		return flag;
	}
	...
	{
		return Mathf.Clamp(rtBuildingIvyContainer.branches[0].totalLength / rtIvyContainer.branches[0].totalLength, 0.1f, 1f);
	}
